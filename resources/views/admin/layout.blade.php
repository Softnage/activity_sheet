...
<div id="sidebar" class="w-64 bg-gray-800 text-gray-200 h-screen fixed md:relative z-20 hidden md:flex flex-col">
    ...
    <nav class="flex flex-col mt-4 space-y-2 px-4">
        ...
        <a href="{{ route('admin.showCreateMeetingForm') }}"
            class="hover:bg-gray-700 p-2 rounded flex items-center {{ request()->routeIs('admin.showCreateMeetingForm') ? 'bg-gray-700' : '' }}">
            <i class="fas fa-calendar-plus mr-3"></i> Create Meeting
        </a>
        <a href="{{ route('admin.storeMeeting') }}"
            class="hover:bg-gray-700 p-2 rounded flex items-center {{ request()->routeIs('admin.storeMeeting') ? 'bg-gray-700' : '' }}">
            <i class="fas fa-calendar-check mr-3"></i> Manage Meetings
        </a>
    </nav>
</div>
